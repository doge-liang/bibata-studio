...
纯 Python 本地 CLI 工具，用于构建 [Bibata](https://github.com/ful1e5/Bibata_Cursor) 鼠标光标主题。没有 Web 栈、没有数据库、没有登录、没有 Figma token —— SVG 进去，光标 `.zip` 出来。
...
那套架构对跑 SaaS 收赞助是合理的，但本地用纯粹是负担 —— 而且 2024 年底开始，它自己也在崩：
...
非交互式、幂等的安装流程，CI、autonomous coding agent、沙箱环境都能直接抄。
...
**Agent 注意事项：**
- `bibata install` 和 `bibata uninstall` 在 Windows 上**需要交互式 UAC 同意**。自动化流程里跳过这两个，用 `bibata build` 出 zip，交给人类用户自己装。
- `bibata preview` 调用 `webbrowser.open()`。headless 环境加 `--no-open`，只打印文件路径。
- 构建是确定性的：相同 `(variant, palette)` 输入 → byte 级相同的 zip（install.inf 注释也是稳定的）。可以放心缓存。
- pip wheel 缓存跨多次运行复用，重复安装只需几秒。
...
上游为每种手向变体准备了四档 hotspot/尺寸配置 —— 选一档，也可以一次全出：
